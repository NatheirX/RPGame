...
function mouseReleased() {
  switch (gameState) {
    case 1:
      ...
      if (
        fitsOnGrid(
          player.squares[targetSquare].x,
          player.squares[targetSquare].y,
          selectedShip.length,
          selectedShip.width
        ) &&
        clearOfShips(
...
function fitsOnGrid(x, y, length, width) {
  //takes in ship.x, ship.y, ship.length, ship.width
  x = Math.round(x / 50);
  y = Math.round(y / 50);
  return x + width < 10 && y + length < 10;
}

function clearOfShips(x, y, length, width, targetSquare) {
  //takes in ship.x, ship.y, ship.length, ship.width
  let clear = true;
  x = Math.round(x / 50);
  y = Math.round(y / 50);
  for (i = 0; i < length; i++) {
    //mark squares as having ship
    for (j = 0; j < width; j++) {
      console.log(targetSquare + j + i * 10);
